...
async function getGasEstimation(tx, address, value) {
    return tx? await tx.estimateGas({ from: address, value: value }): '21000';
}
...
async function executeTransactionBySign(web3Context, contractAddress, tx, value) {
    ...
    const gas = await getGasEstimation(tx, address, value)
    ...
    const signedTx = await web3Context.web3.eth.accounts.signTransaction(txdata,web3Context.pvkey);
    // console.log('transactiond data', {...txdata, hash: signedTx.transactionHash})
...
async function executeContract(web3Context, to, tx, value) {
    // try {
        return await executeTransactionBySign(web3Context, to, tx, value)
    // } catch (err) {
    //     console.log(err)
    // }
}
